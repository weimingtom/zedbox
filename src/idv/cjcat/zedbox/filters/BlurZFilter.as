package idv.cjcat.zedbox.filters {
	...
	public class BlurZFilter implements IZFilter {
		 ...
		 */
		public function BlurZFilter(nearPlane:Number, farPlane:Number, range:Number = 50, blur:Number = 10, quality:int = 1) {
		...
		public function process(data:ZedData):void {
			...
			if (dz < (_nearPlane + _nearRange)) {
				filter.blurX = filter.blurY = ZedBoxMath.clamp(ZedBoxMath.interpolate(_nearPlane + _nearRange, 0, _nearPlane, blur, dz), 0, Number.MAX_VALUE);
				data._zs.filters = [filter];
				return;
			} else if (dz > (_farPlane - _farRange)) {
				filter.blurX = filter.blurY = ZedBoxMath.clamp(ZedBoxMath.interpolate(_farPlane - _farRange, 0, _farPlane, blur, dz), 0, Number.MAX_VALUE);
				data._zs.filters = [filter];
				return;
			}
		}
